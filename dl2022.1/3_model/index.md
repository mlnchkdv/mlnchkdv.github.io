...
output = 
\left\{\begin{matrix}
0 :& \sum_i x_i w_i < threshold \\ 
1 :& \sum_i x_i w_i \geqslant threshold
\end{matrix}\right.
$$
...
output = 
\left\{\begin{matrix}
0 :& \sum_i x_i w_i + bias < 0 \\ 
1 :& \sum_i x_i w_i + bias \geqslant 0
\end{matrix}\right.
$$
...
Ниже приведены только основные функции активации. На деле их достаточно много и все остальные на практике подгоняются под очень тонкие предметные области и задачи. Одной из самых популярных функций активации является сигмоида и ReLU, в то время как функции Хевисайда и Гистерезиса больше применяются в обучающих целях.
...
https://www.geogebra.org/classic/utrd9nnc
